...
const char *useSerialNumber = NULL;
...
COMMAND_HANDLER(handle_serialnum_command)
{
  ...
  if (ERROR_OK == retval)
  {
    useSerialNumber = malloc(strlen(serialNumber)+1);
    if(useSerialNumber)
    {
      strcpy((char *)useSerialNumber, (char *)serialNumber);
...
static int picoprobe_usb_open(void)
{
	...
	if (jtag_libusb_open(vids, pids, useSerialNumber,
